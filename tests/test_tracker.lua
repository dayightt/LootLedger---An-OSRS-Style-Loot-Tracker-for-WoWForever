...
local function load()
    ...
    LL.DB.settings.showUnclaimed = true  -- most tests exercise group tracking
    return LL
end
...
T.test("experience messages name a corpse the unit APIs would not", function()
    local LL = load()
    stub.SetUnits({})  -- nothing resolvable, as in restricted content
    stub.FireEvent("CHAT_MSG_COMBAT_XP_GAIN", "Nalnik dies, you gain 75 experience.")
    T.eq(LL.Tracker.ParseDeathMessage("Nalnik dies, you gain 75 experience. (+15 group bonus)"), "Nalnik")
    T.eq(LL.Tracker.ParseDeathMessage("Nalnik dies, you gain 75 experience. (35 exp Rested bonus)"), "Nalnik")
    T.isnil(LL.Tracker.ParseDeathMessage("You gain 550 experience."))
    seerWindow()
    stub.FireEvent("LOOT_READY", true)
    T.eq(LL.DB.mobs[257532].name, "Nalnik")
    T.eq(LL.DB.npcNames[257532], "Nalnik")
end)

T.test("a mixed pull is ambiguous and leaves the corpse unnamed", function()
    local LL = load()
    stub.SetUnits({})
    stub.FireEvent("CHAT_MSG_COMBAT_XP_GAIN", "Nalnik dies, you gain 75 experience.")
    stub.FireEvent("CHAT_MSG_COMBAT_XP_GAIN", "Ghostly Attendant dies, you gain 70 experience.")
    seerWindow()
    stub.FireEvent("LOOT_READY", true)
...
T.test("a unit name still wins over the experience message", function()
    local LL = load()
    stub.FireEvent("CHAT_MSG_COMBAT_XP_GAIN", "Wrong Name dies, you gain 75 experience.")
    seerWindow()
    stub.FireEvent("LOOT_READY", true)
    T.eq(LL.DB.mobs[257532].name, "Windshaper Novice Seer")
end)

T.test("old experience messages expire", function()
    local LL = load()
    stub.SetUnits({})
    stub.FireEvent("CHAT_MSG_COMBAT_XP_GAIN", "Nalnik dies, you gain 75 experience.")
    stub.Advance(61)
...
T.test("other players' pickups are ignored entirely unless tracking is on", function()
    local LL = load()
    ...
    stub.CloseLoot()
    stub.FireEvent("CHAT_MSG_LOOT", "Bob receives loot: " .. WATER .. ".", "Bob")
    T.deq(LL.DB.mobs[257532].unclaimed, {})
    T.deq(LL.DB.unattributed.unclaimed, {})
    -- Your own loot is unaffected.
    stub.FireEvent("CHAT_MSG_LOOT", "You receive loot: " .. WATER, "Shooty")
    T.eq(LL.DB.mobs[257532].items["159:0"].count, 1)
end)
